...
**빠진 것과 그 이유는 [`src/README.md`](src/README.md) §4에 전부 적어 두었다.** 요약하면 학습된 체크포인트
192 GB, 궤적 데이터 15 GB, 모델 캐시 60 GB, 롤아웃 영상 2.1 GB가 용량 때문에 빠졌고, 빌드가 생성하는
설정·캐시와 실행할 때 생기는 로그·작업 폴더가 빠졌다. 논문 본문과 지도교수의 원 설계서도 저장소에 없다.
자산이 없어도 빌드와 결과 재산출·검증은 되며, GPU 롤아웃 재평가와 무결성 감사만 자산이 필요하다.
