...
> 날짜별 원기록은 [`src/log.md`](src/log.md)다. **수치는** 그 파일들에서 프로그래밍으로 주입했고
> (프로젝트 규칙 = 수치 수동 입력 금지), **서술 문장은 사람이 쓴 것**이다. 둘 다 생성기
> [`src/tools/make_project_story.py`](src/tools/make_project_story.py) 안에 들어 있다.
...
이 연구는 VLA가 성공한 궤적만 모아 상황 묶음마다 가벼운 습관 정책을 학습시키고, 그 습관을 믿어도 되는지 판정하는 관문을 두어 VLA 호출을 선택적으로 생략한다. 습관 정책의 판단 비용은 3.36 ms로 VLA의 4.0% 수준이다.

**설계는 2단 관문(익숙한 상황인가 · 충분히 검증됐는가)이었지만, 온라인 실험에서 실제로 발화를 결정한 것은 성숙도 한 단계뿐이다.** 익숙함 판정이 사전등록 임계(AUC 0.75)를 넘지 못해, 실험 전에 행동에 개입하지 않고 기록만 하는 그림자 로깅으로 강등됐다 (`src/experiments/e5_driver.py` 발화 조건, `src/configs/preregistration.md` §5 2026-08-16). 아래 수치는 전부 그 단일 관문 시스템의 것이다.

결과는 3개 seed × 4,000 에피소드 온라인 스트림에서 나왔다. 조건이 둘이라 함께 적는다.

| 조건 | VLA 호출 비율 (첫→끝 1,000 ep) | 항상 VLA 대비 성공률 차이 | 발화 시 실패 확률 |
|---|---|---|---|
| 사전등록 주 조건 (습관만 깊이 사용) | 0.874 → 0.405 | -0.0021 (CI가 0 포함) | 0.0285 |
| **교락 제거 (교사와 입력 일치)** | 0.8527 → 0.4323 | **-0.0065±0.0014** (세 seed CI 모두 0 아래) | 0.0377 |

허용 한계는 -0.03이고 두 조건 모두 통과한다. 다만 **방법론적으로 깨끗한 쪽은 아래 줄**이고, 거기서는 차이가 0과 구별된다. 습관이 실제로 발화한 구간만 보면 격차는 -0.0161로 더 커진다. 비용을 줄이는 대가가 0은 아니라는 뜻이며, 이 점을 §3.2와 §6에 그대로 적었다.
...
| H3 2단 게이트 | 익숙함 판정과 성숙도 판정의 결합이 각각 단독보다 안전한가 | **사전등록 형태로는 검정 못 함** — 익숙함 단독 arm이 폐지돼 맞비교가 없다 |
...
값어치 있는 부분은 대체 가설 자체가 아니라 **그것을 미리 등록해 둔 것**이다. H2-L′은 E3 집계 전에 사전등록 §4f에 "어느 쪽이 이기든 그대로 보고한다"는 조건과 함께 올라가 있었다. 덕분에 기각이 변명이 아니라 결과가 됐다.

다만 대체 가설의 근거는 강하지 않다. 표본이 22개 셀이고 유의한 예측자는 하나이며 다중비교 보정을 하지 않았다. **원가설이 기각됐다는 것은 단단하고, 무엇이 대신 설명하는지는 잠정이다.**
...
**미해결을 미해결로 확정하고 다음 논문 과제로 넘겼다.** 성숙도 판정 단독으로도 위험 상한은 지켜져 시스템은 그대로 진행할 수 있었다.

대신 대가가 있다. 익숙함 판정을 그림자 로깅으로 내리면서 **익숙함 단독 arm을 폐지**했고, 그 결과 H3가 원래 요구한 "결합이 각 단독보다 나은가"의 맞비교가 저장소에 없다. H3는 부분 확인이 아니라 **사전등록한 형태로는 검정되지 않았다**고 읽는 것이 정확하다 (`src/configs/preregistration.md` §5 2026-08-16).
...
전체 차이의 신뢰구간은 0을 포함했지만 **자동 판정은 CASE B였다** — 궤적 80개 지점의 평균 하락 3.67 pp가 사전에 정한 3 pp 기준을 넘었기 때문이다. 초과분은 사실상 긴 작업 묶음 하나에서 나왔고 그것을 빼면 기준 안이었다. 깨끗한 무차이가 아니라 **신경 쓰이는 신호가 남은 상태**였다.

여기서 정황에 만족하지 않고 **깊이를 뺀 채 전체 프로토콜을 다시 돌렸다.** 무인 실행 약 70시간이 걸렸고, 도중에 감시 프로세스와 자동 재개 장치를 만들어 세션이 끊겨도 진행되게 했다.

## 3. 결과

### 3.1 호출률은 실제로 줄어든다 (H4)
...
### 3.2 깊이를 빼도 사전등록 기준은 통과한다 (다만 부호는 음수)
...
### 3.3 습관을 만드는 비용은 따로 있다

호출률 감소는 **운영 장부** 기준이다. 습관을 만드는 비용은 별도 장부로 계상했고, 지연 주장에 넣지 않았다 (사전등록 §4h의 3장부 분리). 아래는 §3.2의 재실행 자체 실측이다.

| 장부 | 시간 (3 seed 평균) | 지연 주장 산입 |
|---|---|---|
| 운영 (스트림 실행) | 9.16 h | 산입 |
| 형성 (재학습·probe) | 5.55 h | **미산입** |
| 평가 (짝지은 재현) | 비용 미보고 | 미산입 |

재학습은 3 seed 합계 145회 일어났고, 1회가 약 4,834 VLA-호출에 해당한다 (학습분만 4,142, `results/rgb_only_full_rerun_20260828/07_latency_cost/COMPUTE_SUMMARY.json`).

**시간이 아니라 아낀 VLA 추론량 기준으로 보면 격차가 더 크다.** 운영 장부의 시간은 대부분 시뮬레이터 구동이지 이 연구가 줄이겠다는 VLA 추론이 아니다. seed 0에서 습관이 대신 처리한 호출은 24,214건이고, 이를 teacher가 했다면 약 2,069 초가 든다. 그 습관을 만드는 데는 20,427 초를 썼다. **측정 구간 안에서는 약 10배 적자다.**

**따라서 이 결과는 "총 연산량이 줄었다"가 아니라 "호출률이 줄었고 성공률은 유지됐다"로 읽어야 한다.** 상각이 총비용에서 이익으로 넘어가려면 형성 비용을 나눠 갚을 만큼 긴 배치 기간이 필요하고, 그 손익분기는 이 실험의 측정 범위 밖이다.
...
**연구 방향과 설계의 큰 틀은 지도교수가 제시했고, 실무 전반은 내가 수행했다.** 가설과 반증 조건, 사전등록할 수치, 단계별 진행·중단 기준의 골격은 지도교수의 설계서에서 왔다 (원 설계서는 이 저장소에 없고 [`src/CLAUDE.md`](src/CLAUDE.md)가 그 요약이다).

작업은 아래 4층으로 나눠 돌렸고 그 경계를 저장소에 명시해 두었다 ([`src/CLAUDE.md`](src/CLAUDE.md) §6).

| 층 | 담당 | 내용 |
|---|---|---|
| 방향 | 지도교수 | 연구 설계, 사전등록 골격, 주요 갈래의 최종 승인 |
| 실무 | **본인** | 설계의 실험 사양화, 파이프라인 구축·운영, 도구 위임과 그 결과 검증, 진행 중 판단과 보고 |
| 실행 | Claude Code | 코드 작성, 장시간 무인 실행 |
| 분석 판정 | 별도 AI 판정자 | 결과 해석 초안과 판정안 작성 |

내가 한 일의 성격은 "직접 코드를 쳤다"가 아니라 **"위임한 실행을 신뢰할 수 있게 만들었다"** 쪽이다. 설계를 실행 가능한 사양으로 옮기고, 관문과 검증 장치를 짜 넣고, 나온 결과를 그대로 믿지 않고 대조했다. 수치를 손으로 옮기지 못하게 하는 규칙, 재학습마다 런타임에 계약을 검사하는 단언, 산출물을 처음부터 재구성해 대조하는 무결성 감사가 그 장치들이다.

판정자가 올린 분석도 그대로 받지 않았다. 라우팅 이득 산식이 무작위 기각기와 동등해 게이트의 선별성을 가리고 있던 오류를 잡아 정정시킨 것이 그 예다. 정규화 결함으로 완주한 seed를 폐기하고, 저비용 게이트를 미해결로 확정하고, 깊이 교락 의심에 70시간을 더 쓰기로 한 갈래들은 보고와 승인을 거쳐 확정했다.

## 5. 되짚어 볼 판단들

통과한 단계보다 막힌 자리에서의 처리가 더 남는다. 아래는 실무를 맡은 입장에서 문제를 드러내고 대안을 정리해 보고했고, 그 결과 프로젝트가 택한 경로다.

1. **환경 탓으로 넘기지 않고 자기 코드를 의심했다.** 커스텀 환경의 반복 실패를 진단한 끝에 실행기가 낡은 행동을 버리지 않는 결함을 찾았다. 회귀 테스트로 못 박았다.
2. **완주한 실험을 폐기하는 쪽을 택했다.** 정규화 결함을 발견해 보고했고, 첫 seed 결과 전량이 인용 금지로 처리됐다. 증거는 지우지 않고 `INVALID` 표시로 남겼고, 재발 방지 단언을 코드에 심었다.
...
5. **불리한 신호를 덮지 않았다.** 깊이 정보가 특권 정보라는 의심을 선별 검증했더니 기준을 넘는 신호가 남았고, 그 상태로 넘어가지 않고 70시간짜리 전체 재실행으로 조건을 맞췄다. 그 결과 헤드라인 수치가 나빠졌지만 그대로 보고했다.
6. **수치를 손으로 옮기지 않는 규율을 집행했다.** 모든 표와 그림은 결과 JSON에서 프로그래밍으로 생성되게 했다. 이 문서의 수치도 같은 경로로 들어온다.
7. **AI가 낸 분석을 검산했다.** 판정자가 올린 라우팅 이득 산식이 무작위 기각기와 동등하다는 것을 발견해 정정시켰다. 위임의 대가는 검증이라는 것이 이 프로젝트에서 가장 크게 남은 교훈이다.
...
- **총 연산량 기준의 이익은 보이지 못했다.** 형성 비용을 포함한 손익분기는 측정 구간 밖이다 (§3.3).
- **논문 본문과 원 설계서가 이 저장소에 없다.** 집필용 자료와 수치 대조 지시서만 `results/reports/`에 있고, 동기의 원천인 설계서 v1.0은 `src/CLAUDE.md`의 요약으로만 남아 있다.
