...
다만 이 수치는 습관 정책만 깊이 정보를 더 받은 조건에서 나왔다. 교락을 없애려고 입력을 맞춰 전체를 다시 돌린 결과는 **-0.0065±0.0014**로, 사전 등록 한계 안이지만 **세 seed의 신뢰구간이 모두 0 아래**다. 습관이 실제로 발화한 구간만 보면 격차는 -0.0161로 더 커진다. 비용을 줄이는 대가가 0은 아니라는 뜻이며, 이 점을 §3.2와 §6에 그대로 적었다.
...
호출률 감소는 **운영 장부** 기준이다. 습관을 만드는 비용은 별도 장부로 계상했고, 지연 주장에 넣지 않았다 (사전등록 §4h의 3장부 분리).

| 장부 | 시간 (3 seed 평균) | 지연 주장 산입 |
|---|---|---|
| 운영 (스트림 실행) | 9.34±0.24 h | 산입 |
| 형성 (재학습·probe) | 6.07±0.12 h | **미산입** |
| 평가 (짝지은 재현) | 비용 미보고 | 미산입 |

재학습은 seed당 50.7회 일어났고, 1회가 약 2,137 VLA-호출에 해당한다 (`results/e1/e1_latency.json` anchor5). 즉 측정 구간 4,000 에피소드 안에서만 보면 **습관을 만드는 데 든 시간이 운영 시간의 약 65%**다.
