...
def main():
    ...
    W("> 날짜별 원기록은 [`src/log.md`](src/log.md)다. **수치는** 그 파일들에서 프로그래밍으로 주입했고")
    W("> (프로젝트 규칙 = 수치 수동 입력 금지), **서술 문장은 사람이 쓴 것**이다. 둘 다 생성기")
    W("> [`src/tools/make_project_story.py`](src/tools/make_project_story.py) 안에 들어 있다.")
    W("")
    ...
    W("")
    reg = h2.get("regression_formation22", {})
    W("값어치 있는 부분은 대체 가설 자체가 아니라 **그것을 미리 등록해 둔 것**이다. H2-L′은 E3 집계 전에 "
      "사전등록 §4f에 \"어느 쪽이 이기든 그대로 보고한다\"는 조건과 함께 올라가 있었다. "
      "덕분에 기각이 변명이 아니라 결과가 됐다.")
    W("")
    W(f"다만 대체 가설의 근거는 강하지 않다. 표본이 {reg.get('n', 22)}개 셀이고 유의한 예측자는 하나이며 "
      "다중비교 보정을 하지 않았다. **원가설이 기각됐다는 것은 단단하고, 무엇이 대신 설명하는지는 잠정이다.**")
    W("")
    ...
    W("")
    W(f"전체 차이의 신뢰구간은 0을 포함했지만 **자동 판정은 CASE B였다** — 궤적 80개 지점의 평균 하락 "
      f"{abl['mean_drop_pp_at_n80']} pp가 사전에 정한 3 pp 기준을 넘었기 때문이다. "
      "초과분은 사실상 긴 작업 묶음 하나에서 나왔고 그것을 빼면 기준 안이었다. "
      "깨끗한 무차이가 아니라 **신경 쓰이는 신호가 남은 상태**였다.")
    W("")
    W("여기서 정황에 만족하지 않고 **깊이를 뺀 채 전체 프로토콜을 다시 돌렸다.** "
    ...
    W("")
    W("### 3.2 깊이를 빼도 사전등록 기준은 통과한다 (다만 부호는 음수)")
    W("")
    ...
    W("**따라서 이 결과는 \"총 연산량이 줄었다\"가 아니라 \"호출률이 줄었고 성공률은 유지됐다\"로 읽어야 한다.** "
      ...
      "그 손익분기는 이 실험의 측정 범위 밖이다.")
    W("")
    W("### 3.4 결과를 믿을 수 있는 근거")
    W("")
    ...
    W("## 4. 역할과 협업 방식")
    W("")
    W("**연구 방향과 설계의 큰 틀은 지도교수가 제시했고, 실무 전반은 내가 수행했다.** "
      "가설과 반증 조건, 사전등록할 수치, 단계별 진행·중단 기준의 골격은 지도교수의 설계서에서 왔다 "
      "(원 설계서는 이 저장소에 없고 [`src/CLAUDE.md`](src/CLAUDE.md)가 그 요약이다).")
    W("")
    W("작업은 아래 4층으로 나눠 돌렸고 그 경계를 저장소에 명시해 두었다 "
      "([`src/CLAUDE.md`](src/CLAUDE.md) §6).")
    W("")
    W("| 층 | 담당 | 내용 |")
    W("|---|---|---|")
    W("| 방향 | 지도교수 | 연구 설계, 사전등록 골격, 주요 갈래의 최종 승인 |")
    W("| 실무 | **본인** | 설계의 실험 사양화, 파이프라인 구축·운영, 도구 위임과 그 결과 검증, 진행 중 판단과 보고 |")
    W("| 실행 | Claude Code | 코드 작성, 장시간 무인 실행 |")
    W("| 분석 판정 | 별도 AI 판정자 | 결과 해석 초안과 판정안 작성 |")
    W("")
    W("내가 한 일의 성격은 \"직접 코드를 쳤다\"가 아니라 **\"위임한 실행을 신뢰할 수 있게 만들었다\"** 쪽이다. "
      "설계를 실행 가능한 사양으로 옮기고, 관문과 검증 장치를 짜 넣고, 나온 결과를 그대로 믿지 않고 대조했다. "
      "수치를 손으로 옮기지 못하게 하는 규칙, 재학습마다 런타임에 계약을 검사하는 단언, "
      "산출물을 처음부터 재구성해 대조하는 무결성 감사가 그 장치들이다.")
    W("")
    W("판정자가 올린 분석도 그대로 받지 않았다. 라우팅 이득 산식이 무작위 기각기와 동등해 "
      "게이트의 선별성을 가리고 있던 오류를 잡아 정정시킨 것이 그 예다. "
      "정규화 결함으로 완주한 seed를 폐기하고, 저비용 게이트를 미해결로 확정하고, "
      "깊이 교락 의심에 70시간을 더 쓰기로 한 갈래들은 보고와 승인을 거쳐 확정했다.")
    W("")

    # ---------------------------------------------------------------- 5. 판단
    W("## 5. 되짚어 볼 판단들")
    W("")
    W("통과한 단계보다 막힌 자리에서의 처리가 더 남는다. 아래는 실무를 맡은 입장에서 "
      "문제를 드러내고 대안을 정리해 보고했고, 그 결과 프로젝트가 택한 경로다.")
    W("")
    W("1. **환경 탓으로 넘기지 않고 자기 코드를 의심했다.** 커스텀 환경의 반복 실패를 진단한 끝에 "
      "실행기가 낡은 행동을 버리지 않는 결함을 찾았다. 회귀 테스트로 못 박았다.")
    W("2. **완주한 실험을 폐기하는 쪽을 택했다.** 정규화 결함을 발견해 보고했고, 첫 seed 결과 전량이 "
      "인용 금지로 처리됐다. 증거는 지우지 않고 `INVALID` 표시로 남겼고, 재발 방지 단언을 코드에 심었다.")
    ...
    W("5. **불리한 신호를 덮지 않았다.** 깊이 정보가 특권 정보라는 의심을 선별 검증했더니 기준을 넘는 "
      "신호가 남았고, 그 상태로 넘어가지 않고 70시간짜리 전체 재실행으로 조건을 맞췄다. "
      "그 결과 헤드라인 수치가 나빠졌지만 그대로 보고했다.")
    W("6. **수치를 손으로 옮기지 않는 규율을 집행했다.** 모든 표와 그림은 결과 JSON에서 프로그래밍으로 "
      "생성되게 했다. 이 문서의 수치도 같은 경로로 들어온다.")
    W("7. **AI가 낸 분석을 검산했다.** 판정자가 올린 라우팅 이득 산식이 무작위 기각기와 동등하다는 것을 "
      "발견해 정정시켰다. 위임의 대가는 검증이라는 것이 이 프로젝트에서 가장 크게 남은 교훈이다.")
    W("")
    ...
    W("- **총 연산량 기준의 이익은 보이지 못했다.** 형성 비용을 포함한 손익분기는 측정 구간 밖이다 (§3.3).")
    W("- **논문 본문과 원 설계서가 이 저장소에 없다.** 집필용 자료와 수치 대조 지시서만 `results/reports/`에 있고, "
      "동기의 원천인 설계서 v1.0은 `src/CLAUDE.md`의 요약으로만 남아 있다.")
    W("")
